 ...
 //print_r($phase);die;
   ...
   <!--  <td><?= $values->price?></td> -->
	...
	<td style="width:100px; text-align:center;">		
	   <button onclick="return rowstatus('<?=TRIPIMAGE?>','id','<?=$values->id?>','status','<?=$values->status?>');"  title="Change Status" <?php if($values->status == '1'){ ?> class="btn btn-success btn-xs"> <?php echo "Approved "; } else { ?> class="btn btn-primary btn-xs"><?php  echo "Unapproved"; } ?>
		</button> 
	</td>
		...
		<div class="btn-group">
		  <button type="button" class="btn btn-danger btn-xs maction dropdown-toggle" data-toggle="dropdown">Action</button>
			...
			<ul class="dropdown-menu" role="menu">
			  <li>
				<a href="javascript:void(null);" onclick="return deleterowAll('<?=strrev(TRIPIMAGE)?>','id','<?=$values->id?>');"><i class="fa fa-trash" aria-hidden="true" class="btn"></i>Delete</a>
				</li>
    ...
    <div class="modal-dialog modalstyle">    
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Trip Image Edit</h4>
        </div>
        <form action="<?php echo base_url('admin/subtrip/editimage')?>" enctype="multipart/form-data" method="post">
