...
class Payment extends Admin_Controller
{
	...
	public function index($slug='')
  {
    ...
    $getTripidsmain = $this->My_model->fetchValue(SUBTRIP,'id',array('slug' =>$slug));
  ...
  //  print_r($data['domestic_tour']);die;
    ...
    $data['all_package'] = $this->My_model->getfields(SUBTRIP,'*',array('id' =>$getTripidsmain));
   // echo "<pre>";
  ...
  public function successform()
    {
      $razorpayIDS   =  $this->input->post('razorpay_payment_id');
      ...
      $noofchildrens = $this->input->post('noofchildrens');
      $razorpay_status  = $this->input->post('razorpay_status');
      $servicename = $this->My_model->fetchValue(SUBTRIP,'subtitle',array('id' =>$product_id));
       $settings = $this->My_model->getfields(SETTING,'*',array('status'=>1,'id'=>1),"");
      $serviceData =  array(
                     'supackage_id'        => $product_id,
                     'razorpay_payment_id' => $razorpayIDS,
                     'name'                => $name,
                     'email'               => $email,
                     'phone'               => $phone,
                     'address'             => $address,
                     'departure_date'      => date("Y-m-d",strtotime($ddate)),
            ...
            $id = $this->My_model->insert_data('md_payment',$serviceData);
            if($id){
                $this->msgsuccess("Payment has been made successfully..");
                
                 $htmlform ='<html>
                             <table border="0" width="700px">
                               <tr><td colspan="2"><h3>Dear user your payment has been done,Details of which are following:.</h3><td></tr>
                               <tr><th>Name</th><td>'.$serviceData['name'].'</td></tr>
                               <tr><th>Email</th><td>'.$serviceData['email'].'</td></tr>
                                <tr><th>Phone</th><td>'.$serviceData['phone'].'</td></tr>
                                <tr><th>Departure Date</th><td>'.$serviceData['departure_date'].'</td></tr>
                                <tr><th>Amount</th><td>INR '.$serviceData['amount'].'</td></tr>
                                <tr><th>Service Name</th><td>'.$servicename.'</td></tr>
                                <tr><th>Number of adult</th><td>'.$serviceData['noofadults'].'</td></tr>
                                <tr><th>Number of children</th><td>'.$serviceData['noofchildren'].'</td></tr>
                                <tr><td></td><td><h3>Thanks & Regards </h3></td></tr>
                                <tr><td></td><td><h3>DreamMyTrip Tourism India Pvt Ltd. </h3></td></tr>
                             </table>
                         </html>';
             $this->load->library('email');
             $this->email->from($serviceData['email'])
                  ->to($settings[0]->email)
                  ->cc($serviceData['email'])
                  ->subject("Payment successful by DreamMyTrip Tourism")
                  ->message($htmlform)
                  ->set_mailtype('html')
                  ->send();
              echo 'success';
//$this->msgsuccess("Payment has been made successfully..");
        ...
        {
            $this->msgwarning("Payment has been made successfully..");
            echo "failure";
            // $this->msgerror("payment has not been done.");
           // redirect('home'); 
        }   
    }
