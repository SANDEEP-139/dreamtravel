...
//$salt = 'wia56q6O';
$adh_redirect =$_SESSION['adh_redirect'];
...
if (isset($postdata ['key'])) {
	...
	if ($status == 'success'  && $resphash == $CalcHashString) {
		...
		$pchecksum = encrypt('payment_status=Confirmed');
		$redirect =$adh_redirect.'&pchecksum='.$pchecksum.'&txn_number='.$mihpayid;

		//$msg = "Transaction Successful, Hash Verified by...<br />";
		...
		$pchecksum = encrypt('payment_status=Failed');
	    $redirect =$adh_redirect.'&pchecksum='.$pchecksum.'&txn_number='.$mihpayid;
	} 
}
//else exit(0);
...
function createLogFile($ORDERID,$request,$response){
    $log_filename = "/var/www/html/demo/b-team/PayUBiz/LogFile";
